/**
 * @brief Handle on the Combase library context
 * @author Anh Quan Nguyen <quannaus@users.sourceforge.net>
 * @date 20/01/2010
 * @detail Combase aims at providing a clean API, and the corresponding
 ...
 *
 *         Install the Combase library for Airline IT Standard C++ fundaments.
 */
...
#include <string>
// ComBase
#include <combase/combase_service_types.hpp>
#include <combase/basic/BasLogParams.hpp>
#include <combase/basic/BasDBParams.hpp>
#include <combase/basic/ServiceInitialisationType.hpp>
...
namespace combase {
  ...
  class combase_service_context;
  

  /**
   * @brief Interface for the COMBASE Services
   */
  ...
  public:
     ...
     */
    combase_service (const BasLogParams&);
     ...
     */
    combase_service (const BasLogParams&, const BasDBParams&);
  ...
  public:
     ...
     *
     * @param const combase::AirlineCode_T& Airline code of the flight to check.
     * @param const combase::FlightNumber_T& Flight number of the
     *        flight to check.
     * @param const combase::Date_T& Departure date of the flight to check.
     ...
     *
     * @param const combase::AirportCode_T& Origin airport of the fare
     *        rule to check.
     * @param const combase::AirportCode_T& Destination airport of the
     *        fare rule to check.
     * @param const combase::Date_T& Departure date of the fare rule
     *        to check. 
  ...
  public:
     ...
     * combase_service instance) has gone through.
     *
  ...
  private:
     ...
     *
     * @param[in] const combase_service& Reference on the
     *            combase_service handler to be copied.
     */
    combase_service (const combase_service&);
    
    /**
     * @brief Initialise the (TRADEMGEN) service context (i.e., the
     * combase_service_context object).
     */
  ...
  private:
     ...
     * Combase context.
     */
    combase_service_context* _combaseServiceContext;
  };
}
